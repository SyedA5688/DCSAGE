...
class DCGCN(torch.nn.Module):
    """
    DCSAGE with GraphSAGE layers replaced by GCNConv

    Constructor Arguments:
        node_features: number of features each node in 1-day graph contains
        emd_dim: embedding dimension that GCN layers will output
    ...
    def __init__(self,
        ...
        self.gcn1 = GCNConv(in_channels=node_features, out_channels=self.emb_dim, normalize=False )
        self.gcn2 = GCNConv(in_channels=self.emb_dim, out_channels=self.emb_dim, normalize=False )
    ...
    def forward(self, data: Data, h_1: Tensor=None, c_1: Tensor=None, 
        ...
        # First GNN Layer
        x = self.gcn1(x, edge_index=edge_index, edge_weight=edge_attr)
        x = self.graph_norm_1(x)
        x = F.relu(x)
        gcn_outputs.append(x)
        
        x = self.gcn2(x, edge_index=edge_index, edge_weight=edge_attr)
    ...
    def visualize_spatial_embeddings(self, args, data_loader, split_name, save_path, epoch_idx):
        ...
        for day_idx, (day_node_feat, day_edge_idx, day_edge_attr) in enumerate(zip(data_loader.dataset.orig_feature_matrix, data_loader.dataset.orig_edge_idx, data_loader.dataset.orig_edge_weights)):
            ...
            # First GNN Layer
            x = self.gcn1(day_node_feat, day_edge_idx, day_edge_attr)  # x becomes [10, self.emb_dim]
            spatial_embeddings.append(x)  # Append before ReLU(), straight from gcn layer
            # x = self.batch_norm_1(x)
            x = F.relu(x)
            
            x = self.gcn2(x, day_edge_idx, day_edge_attr)  # x becomes [10, self.emb_dim]
            spatial_embeddings.append(x)
            # x = self.batch_norm_2(x)
            x = F.relu(x)

            # Concatenate spatial feature outputs of both gcn layers; local and more global spatial info
            x = torch.cat(spatial_embeddings, dim=1)  # x becomes [10, 2 * self.emb_dim]

            # Apply TSNE reductionality to gcn 1, gcn 2, and concatenated spatial embeddings
            gcn1_reduced_emb = TSNE(n_components=2, learning_rate='auto', init='random').fit_transform(spatial_embeddings[0].detach().numpy())  # [10, 2]
            gcn2_reduced_emb = TSNE(n_components=2, learning_rate='auto', init='random').fit_transform(spatial_embeddings[1].detach().numpy())
            gcn_concat_reduced_emb = TSNE(n_components=2, learning_rate='auto', init='random').fit_transform(x.detach().numpy())
            gcn1_emb_list.append(np.expand_dims(gcn1_reduced_emb, axis=0))
            gcn2_emb_list.append(np.expand_dims(gcn2_reduced_emb, axis=0))
            gcn_concat_emb_list.append(np.expand_dims(gcn_concat_reduced_emb, axis=0))
        
        # Concatenate embedding lists into single numpy arrays
        gcn1_emb_list = np.concatenate(gcn1_emb_list, axis=0)
        gcn2_emb_list = np.concatenate(gcn2_emb_list, axis=0)
        gcn_concat_emb_list = np.concatenate(gcn_concat_emb_list, axis=0)

        # Plot embeddings
        if not os.path.exists(os.path.join(save_path, "spatial_emb_plots")):
            os.mkdir(os.path.join(save_path, "spatial_emb_plots"))
        
        plot_tsne_reduced_embeddings(gcn1_emb_list, plot_title="gcn Layer 1 Embeddings Epoch " + str(epoch_idx) + " " + split_name + " Dataset", save_filename=split_name + "_gcn1_emb_epoch_" + str(epoch_idx), SAVE_PATH=os.path.join(save_path, "spatial_emb_plots"))
        plot_tsne_reduced_embeddings(gcn2_emb_list, plot_title="gcn Layer 2 Embeddings Epoch " + str(epoch_idx) + " " + split_name + " Dataset", save_filename=split_name + "_gcn2_emb_epoch_" + str(epoch_idx), SAVE_PATH=os.path.join(save_path, "spatial_emb_plots"))
        plot_tsne_reduced_embeddings(gcn_concat_emb_list, plot_title="gcn Concat Embeddings Epoch " + str(epoch_idx) + " " + split_name + " Dataset", save_filename=split_name + "_concat_emb_epoch_" + str(epoch_idx), SAVE_PATH=os.path.join(save_path, "spatial_emb_plots"))
    ...
    def visualize_activations(self, args, data_loader, split_name, save_path, epoch_idx, color="C0"):
            ...
            for batch_window_node_feat, batch_window_edge_idx, batch_window_edge_attr, batch_window_labels in data_loader:
                for count, window_idx in enumerate(range(len(batch_window_node_feat))):
                    ...
                    for day_idx in range(len(window_node_feat)):
                        ...
                        # First GNN Layer
                        x = self.gcn1(day_node_feat, day_edge_idx, day_edge_attr)
                        ...
                        x = self.gcn2(x, day_edge_idx, day_edge_attr)
        ...
        activations = {}
        activations["gcn1"] = np.array(gs_1_activations_list).flatten()
        activations["GraphNorm1"] = np.array(graphnorm_1_activations_list).flatten()
        activations["gcn2"] = np.array(gs_2_activations_list).flatten()
        activations["GraphNorm2"] = np.array(graphnorm_2_activations_list).flatten()
        activations["gcnConcat"] = np.array(gs_concat_activations_list).flatten()
