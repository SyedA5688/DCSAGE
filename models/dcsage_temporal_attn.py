...
class Temporal_Attention(nn.Module):
    def __init__(self, outfeat, k, d):
        """
        Temporal attention layer taken from https://github.com/LMissher/ST-GRAT/blob/main/model.py
        ...
        self.qff = nn.Linear(outfeat, outfeat)
        self.kff = nn.Linear(outfeat, outfeat)
        self.vff = nn.Linear(outfeat, outfeat)

        # self.ff = nn.Sequential(
        #     nn.Linear(outfeat, outfeat),
        #     nn.GELU(),
        #     nn.Linear(outfeat, outfeat),
        # )

        # self.ln = nn.LayerNorm(outfeat)

        self.d = d
        self.k = k

    def forward(self, x, Mask=False):
        x = x.unsqueeze(0)  # fake batch dimension
        query = self.qff(x)
        key = self.kff(x)
        value = self.vff(x)

        query = torch.cat(torch.split(query, self.d, -1), 0).permute(0,2,1,3)  # (batch*n_heads, num_nodes, seq_len, dim)
        key = torch.cat(torch.split(key, self.d, -1), 0).permute(0,2,3,1)  # # (batch*n_heads, num_nodes, dim, seq_len)
        value = torch.cat(torch.split(value, self.d, -1), 0).permute(0,2,1,3)  # (batch*n_heads, num_nodes, seq_len, dim)

        a = torch.matmul(query, key)  # (batch*n_heads, num_nodes, seq_len, seq_len)
        a /= (self.d ** 0.5)
        ...
        if Mask == True:
            ...
            zero_vec = (-2 ** 15 + 1)*torch.ones_like(a).to(device) # [k*B,N,T,T]
            a = torch.where(mask, a, zero_vec)

        a = torch.softmax(a, -1)  # softmaxing over last dim of (b*n_heads, nodes, seq, seq) => last dim is attn coefficients

        value = torch.matmul(a, value)  # (b*n_heads, nodes, seq, seq) * (b*n_heads, nodes, seq, dim)
        value = torch.cat(torch.split(value, x.shape[0], 0), -1).permute(0,2,1,3)
        # value = self.ff(value) + x  # Post-LN transformer scaled dot product multihead attention. LayerNorm comes after skip

        # return self.ln(value).squeeze(0), a  # (b, seq_len, num_nodes, emb_dim), (batch*n_heads, nodes, seq_len, seq_len)
        # value = self.ff(value)
        # value = self.ln(value) + x  # Pre-LN layer, LayerNorm comes before skip connection
        return value.squeeze(0), a



class DCSAGE_Temporal_Attn(torch.nn.Module):
    def __init__(self,
                node_features: int = 2, 
                emb_dim: int = 10,
                window_size: int = 14,
                ...
                name: str = "DCSAGE_Temporal_Attn"):
        super(DCSAGE_Temporal_Attn, self).__init__()
        ...
        # self.temporal_attn1 = Temporal_Attention(outfeat=10, k=1, d=10)
        self.temporal_attn2 = Temporal_Attention(outfeat=10, k=2, d=5)  # 2 attention heads, each dim 5
    ...
    def forward(self, data: Data, h_1: Tensor=None, c_1: Tensor=None, 
        ...
        self.h_1_list.append(h_1.unsqueeze(0))
        self.h_2_list.append(h_2.unsqueeze(0))
        h1_alphas, h2_alphas = None, None
        
        if day_idx == self.window_size - 1:
            # Skip connection
            concat_feat = torch.cat(self.concat_feat_list, dim=1)  # becomes [10, 16]
            # h1_concat = torch.cat(self.h_1_list, dim=0)
            h2_concat = torch.cat(self.h_2_list, dim=0)

            # h1_concat, h1_alphas = self.temporal_attn1(h1_concat)
            h2_concat, h2_alphas = self.temporal_attn2(h2_concat)

            # h1_alphas is (k, 10, 7, 7) = (batch*n_heads, nodes, seq, seq)
            # Average the attention coefficients of the 5 attention heads together,
            # then average across nodes
            # h1_alphas = h1_alphas.sum(dim=0).sum(dim=0)  # (7, 7)
            h2_alphas = h2_alphas.sum(dim=0).sum(dim=0)

            x = torch.cat((concat_feat, h_1, h2_concat[-1]), dim=1)  # h1_concat[-1]
        ...
        return x, h_1, c_1, h_2, c_2, (h1_alphas, h2_alphas)
